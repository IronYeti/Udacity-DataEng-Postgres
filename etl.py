...
def process_song_file(cur, filepath):
    """
    ETLs a song data file into database (songs and artists tables)
    
    Parameters:
    cursor (obj): current database cursor
...
def process_log_file(cur, filepath):
    """
    Loads the log files into database
    ...
    # load user table
    user_data = pd.concat([df['userId'], df['firstName'], df['lastName'], df['gender'], df['level']], axis=1)
...
def process_data(cur, conn, filepath, func):
    """
    Processes the song and log files into the database
    
    Parameters:
    cursor (obj): current database cursor
    conn (obj): current database connection
    filepath (str): path to the song file
    func (functionObj): function name to execute
